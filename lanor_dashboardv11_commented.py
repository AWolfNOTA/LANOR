...
addr = 0
#Function to set addresses appropriately for Pi Plates stacked on top of one another
def initADDR3():
    global addr
    addrSet=False
...
'''
#Function to set and enable to PWM voltage used to turn on LEDs 
def LED(val):
    #Turn on LEDS if percentage is above 0 
    if val != 0:
        #Set LEDs connected to motor driver 4 to the value given as an argument
        MOTOR.dcSPEED(1, 4, val)
        #Turn on LEDs connected to motor driver 4
        MOTOR.dcSTART(1,4)
        #Set LEDs connected to motor driver 3 to the value given as an argument
        MOTOR.dcSPEED(1, 3, val)
        #Turn on LEDs connected to motor driver 3
        MOTOR.dcSTART(1,3)
    #Turn off LEDs if percentage is 0 
    else:
        MOTOR.dcSTOP(1, 4)
        MOTOR.dcSTOP(1,3)


#Function to calculate gas ppm values from the sensor readings of NO and NO2
...
class Graph:
    ...
    #Function tied to "Start Graph" button to create canvas and start graph  
    def startgraph(self):
        self.start_time = time.time()
        self.xs.clear()
        self.ys.clear()
    ...
    #Function tied to "Stop Graph" Button
    def stopgraph(self):
        self.gs=0
        
    #Function to add each new value to the graph 
    ...
    #Function to redraw graph for animation 
...
class Log:
    ...
    def startlog(self):
        #Create headers if PID values are included 
        if self.pl.get() == 1:
            data = "Time (s), NO (ppm), NO2 (ppm), DAC (V), P, I, D\n"
        #Create headeres if PID values are not included 
        else:
            data = "Time (s), NO (ppm), NO2 (ppm)\n"
        #Create file and append headers 
        if self.ls.get() == 1:
            print("starting")
            ...
            if not os.path.isfile(logfile):
                ...
                with open(logfile,'a+') as f:
                    f.write(data)
    #Function to add a row of information to the log file -- called in the task loop while program is running 
...
class AutoPage(tk.Frame):
    ...
    def get_PID(self, no_ppm):
        ...
        if self.controller.mode == 1:
            ...
            #Set PWM voltage percent for LEDs based on the results of the PID calculation 
            LED(percent)
            return light_val
        else:
